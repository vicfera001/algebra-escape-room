      ...
      >
        <div className="p-8">
          ...
          <div className="flex flex-col items-center mb-8">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${success ? 'bg-green-500/20 text-green-500' : 'bg-destructive/20 text-destructive'}`}>
              {success ? <Unlock size={32} /> : <Lock size={32} />}
            </div>
            <h2 className="text-2xl font-mono font-bold tracking-widest text-foreground">ELECTRONIC LOCK</h2>
          ...
          {!hasAllClues && !success && (
            <div className="bg-orange-500/10 border border-orange-500/30 text-orange-200 text-sm p-4 rounded-lg mb-6 text-center">
              Find all 3 digits in the classroom before attempting to unlock.
            </div>
          )}
              ...
              {[0, 1, 2].map((i) => (
                <input
                  ...
                  className={`w-16 h-20 text-4xl text-center font-mono font-bold rounded-lg border-2 bg-black text-primary shadow-inner
                    ${errorState ? 'border-destructive text-destructive' : 
                      success ? 'border-green-500 text-green-500' : 'border-border focus:border-primary'} 
            ...
            <button
              ...
              className={`w-full py-4 rounded-lg font-bold font-mono text-xl tracking-widest transition-all
                ${success ? 'bg-green-500 text-black' : 
                  code.join('').length === 3 ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 
                  'bg-muted text-muted-foreground cursor-not-allowed'}
              `}
            >
              {success ? 'UNLOCKED' : 'ENTER'}
            </button>
          </form>
        </div>
      </motion.div>
